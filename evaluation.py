...
class AdaptiveSoftmaxLoss(nn.Module):
    def __init__(self, alpha=0.9):
        super(AdaptiveSoftmaxLoss, self).__init__()
        self.alpha = alpha

    def forward(self, outputs, targets):
        log_probs = F.log_softmax(outputs, dim=1)
        loss = -torch.sum(targets * log_probs)
        grads = torch.autograd.grad(loss, outputs, create_graph=True)[0]
        grad_magnitude = grads.norm(p=2, dim=1)
        weight = grad_magnitude ** self.alpha
